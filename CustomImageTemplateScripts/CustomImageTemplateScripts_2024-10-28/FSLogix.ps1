...
Param (        
    # Use this if your FSLogix profile is stored in Azure Files
    [Parameter(Mandatory=$false)]
        [string]$ProfilePath,

    # Use this if your FSLogix profile is stored in Azure Page Blob (YOu can pass an fslogix secure key here)
    [Parameter(Mandatory=$false)]
        [string]$CCDLocations,        

    # If you are using the secure key for CCDLocations, pass the connection string here and it will be encrypted 
    [Parameter(Mandatory=$false)]
...
if(($PSBoundParameters.ContainsKey('ConnectionString'))) {
    & "C:\Program Files\FSLogix\Apps\frx.exe" add-secure-key -key page-store -value $ConnectionString
}
...
if(($PSBoundParameters.ContainsKey('CCDLocations'))) {
    New-ItemProperty `
    -Path HKLM:\Software\FSLogix\Profiles `
    -Name "CCDLocations" `
    -Value "type=azure,connectionString=$CCDLocations" `
    -PropertyType MultiString `
    -Force
}
